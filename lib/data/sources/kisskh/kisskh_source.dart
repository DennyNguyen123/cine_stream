...
class KissKhSource implements MovieSource {
  ...
  Future<List<HomeSection>> getHomeSections() async {
    final trending = await _api.getDramaList(
      page: 1,
      type: 1,
      order: 1,
    ); // TV Series
    final popularMovie = await _api.getDramaList(
      page: 1,
      type: 2,
      order: 1,
    ); // Movie
    final recentlyUpdated = await _api.getDramaList(
      page: 1,
      type: 0,
      order: 2,
    ); // Last update
  ...
  Future<StreamInfo?> getStreamInfo(
    ...
    try {
      ...
      final String? streamKkey = await _kkeyExtractor.extractStreamKey(
        parsedMovieId,
        parsedEpisodeId,
      );
      ...
      final String? subKkey = await _kkeyExtractor.extractSubKey(
        parsedMovieId,
        parsedEpisodeId,
      );
      ...
      final streamInfo = StreamInfo(videoUrl: videoUrl, subtitles: tracks);
