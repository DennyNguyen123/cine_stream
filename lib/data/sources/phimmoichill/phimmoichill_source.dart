...
class PhimMoiChillSource implements MovieSource {
  ...
  Future<List<HomeSection>> getHomeSections() async {
    ...
    return [
      HomeSection(
        title: 'Mới cập nhật',
        movies: updated?.items.map((e) => Movie(id: e.slug, title: e.name, thumbnail: e.thumbUrl, type: e.displayStatus, status: e.status)).toList() ?? [],
      ),
      HomeSection(
        title: 'Thịnh hành',
        movies: trending?.items.map((e) => Movie(id: e.slug, title: e.name, thumbnail: e.thumbUrl, type: e.displayStatus, status: e.status)).toList() ?? [],
      ),
      HomeSection(
        title: 'Mới đăng',
        movies: favorites?.items.map((e) => Movie(id: e.slug, title: e.name, thumbnail: e.thumbUrl, type: e.displayStatus, status: e.status)).toList() ?? [],
      ),
    ];
  }
  ...
  Future<List<Movie>> searchMovies(String query) async {
    ...
    return result?.items.map((e) => Movie(
      id: e.slug,
      title: e.name,
  ...
  Future<MovieDetail?> getMovieDetail(String id) async {
    ...
    for (int i = 0; i < rawEpisodes.length; i++) {
      String epUrl = rawEpisodes[i]['url'] ?? '/xem-phim/$id/${rawEpisodes[i]['slug']}';
    ...
    rawDesc = rawDesc.replaceAll('&quot;', '"').replaceAll('&amp;', '&').replaceAll('&#039;', "'").replaceAll('&lt;', '<').replaceAll('&gt;', '>');
    rawTitle = rawTitle.replaceAll('&quot;', '"').replaceAll('&amp;', '&').replaceAll('&#039;', "'").replaceAll('&lt;', '<').replaceAll('&gt;', '>');
