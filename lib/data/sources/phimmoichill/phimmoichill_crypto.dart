...
class PhimmoichillCrypto {
  static final Uint8List _keyTable = Uint8List.fromList([
  ...
  static final Uint8List _invTable = _getInvTable();

  static Uint8List _getInvTable() {
  ...
  static int _calcVal(int seed, int loops) {
    var v = seed & 0xff;
    for (var i = 0; i <= loops; i++) {
      v = _keyTable[v];
      v = (v ^ (loops * 37 + i * 13 + 7) & 0xff) & 0xff;
    }
    return v;
  }
  ...
  static Uint8List decrypt(Uint8List data, String transportKeyHex) {
    if (data.isEmpty) return data;

    if (data[0] == 0xff && data.length > 17) {
      final iv = data.sublist(1, 17);
      final body = Uint8List.fromList(data.sublist(17));
      final transportKeyBytes = Uint8List.fromList(_hexToBytes(transportKeyHex));

      for (var i = 7; i >= 0; i--) {
        final v1 = _calcVal(iv[i & 0xf], i);
        final v2 = (i * 17) & 0xff;
        for (var j = 0; j < body.length; j++) {
          var b = body[j];
          b = ((b >> 3) | (b << 5)) & 0xff;
          b = _invTable[b];
          b = (b ^ (v1 + j * 13 + v2) & 0xff) & 0xff;
          b ^= transportKeyBytes[j & 0xf];
          body[j] = b;
        }
      }
      return body;
