...
class VidsrcmeExtractor {
  static Future<StreamInfo?> extractStream(
    ...
    Future<String?> resolveRedirect(
      ...
      print(
        '[VidsrcmeExtractor] resolveRedirect: waiting 1s for WebView to release connection...',
      );
      ...
      try {
        ...
        print(
          '[VidsrcmeExtractor] resolveRedirect status: ${response.statusCode}',
        );

        if (response.statusCode >= 400) {
          print(
            '[VidsrcmeExtractor] 🛑 Invalid status code: ${response.statusCode}',
          );
          return null;
        }
    ...
    headlessWebView = HeadlessInAppWebView(
      ...
      shouldOverrideUrlLoading: (controller, navigationAction) async {
        ...
        if (navigationAction.isForMainFrame) {
          if (!url.contains('vidsrcme.ru') &&
              !url.contains('about:') &&
              !url.contains('cloudorchestranova.com') &&
              !url.contains('vidsrc')) {
      ...
      initialUserScripts: UnmodifiableListView<UserScript>([
        ...
        UserScript(
          ...
          injectionTime: UserScriptInjectionTime
              .AT_DOCUMENT_START, // Inject sớm nhất có thể
        ...
        controller.addJavaScriptHandler(
          ...
          callback: (args) async {
            if (args.isNotEmpty) {
              ...
              print(
                '[VidsrcmeExtractor] ➡ Redirecting via JS Handler to: $iframeSrc',
              );
      ...
      onCreateWindow: (controller, createWindowAction) async {
        print(
          '[VidsrcmeExtractor] ⚠ Blocked popup window: \${createWindowAction.request.url}',
        );
        return false;
      },
      onReceivedError: (controller, request, error) {
        print(
          '[VidsrcmeExtractor] Error: ${request.url} - ${error.description}',
        );
      },
      shouldInterceptRequest: (controller, request) async {
        ...
        return _handleNetworkRequest(
          ...
          currentUrl?.toString() ?? 'https://vidsrcme.ru/',
          completer,
          timeoutTimer,
          subtitles,
          resolveRedirect,
      ...
      onJsConfirm: (controller, jsConfirmRequest) async => JsConfirmResponse(
        handledByClient: true,
        action: JsConfirmResponseAction.CONFIRM,
      ),
      onJsPrompt: (controller, jsPromptRequest) async => JsPromptResponse(
        handledByClient: true,
        action: JsPromptResponseAction.CONFIRM,
      ),
      onJsBeforeUnload: (controller, jsBeforeUnloadRequest) async {
        print(
          '[VidsrcmeExtractor] ⚠ Blocked main frame redirect (beforeunload): \${jsBeforeUnloadRequest.url}',
        );
    ...
    if (isValidStream) {
      ...
      if (!completer.isCompleted) {
        ...
        CookieManager.instance()
            .getCookies(url: WebUri('https://vidsrcme.ru/'))
            .timeout(const Duration(seconds: 2))
            .then((iframeCookies) {
              CookieManager.instance()
                  .getCookies(url: WebUri(reqUrl))
                  .timeout(const Duration(seconds: 2))
                  .then((streamCookies) {
                    ...
                    if (!completer.isCompleted) {
                      ...
                      resolveRedirect(reqUrl, headers).then((finalUrl) {
                        if (finalUrl != null && !completer.isCompleted)
                          completer.complete(
                            StreamInfo(
                              videoUrl: finalUrl,
                              subtitles: subtitles,
