...
class ExternalSubtitleRepository {
  ...
  Future<List<SubtitleTrack>> getSubtitles(
    String rawId, {
    int? season,
    int? episode,
  }) async {
    try {
      ...
      if (!imdbId.startsWith('tt')) {
        debugPrint(
          'Stremio addons usually require IMDb ID starting with tt. Got: $imdbId',
        );
        return [];
      }
      ...
      final sources = [
        ...
        if (!isTv)
          'https://yifysubtitles.strem.io/subtitles/$path', // YTS (movies only)
      ];
          ...
          if (response.statusCode == 200 && response.data != null) {
            ...
            if (data is Map && data['subtitles'] != null) {
              ...
              for (var i = 0; i < results.length; i++) {
                final sub = results[i];
                final rawLang =
                    sub['lang']?.toString().toLowerCase() ?? 'unknown';
