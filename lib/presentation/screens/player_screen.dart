...
class _PlayerScreenState extends State<PlayerScreen>
  ...
  void initState() {
    ...
    getIt<LogService>().log(
      '[PlayerScreen] Initializing with videoUrl: ${widget.streamInfo.videoUrl}',
    );
    getIt<LogService>().log(
      '[PlayerScreen] Initializing with headers: ${widget.streamInfo.headers}',
    );
    ...
    if (isHttp || isFileUri || hasHeaders) {
      ...
      _controller = VideoPlayerController.networkUrl(
        videoUri,
        formatHint: widget.streamInfo.videoUrl.contains('.mp4')
            ? VideoFormat.other
            : VideoFormat.hls,
        httpHeaders:
            widget.streamInfo.headers ??
            {
              'User-Agent':
    ...
    _initTimeoutTimer = Timer(const Duration(seconds: 15), () {
      if (mounted && !_controller.value.isInitialized && !_errorDialogShowing) {
        getIt<LogService>().log(
          '[PlayerScreen] Video initialization timeout after 15s.',
        );
        _handlePlayerError(
          'Connection timeout. Please check your network or try again.',
        );
      }
    });
  ...
  Future<void> _fetchExternalSubtitles() async {
    ...
    try {
      ...
      if (isSeries && widget.allEpisodes.isNotEmpty) {
        final ep = widget.allEpisodes.firstWhere(
          (e) => e.id == widget.episodeId,
          orElse: () => widget.allEpisodes.first,
        );
        season = ep.season;
        episode = ep.number.toInt();
      }

      final extraSubs = await extRepo.getSubtitles(
        widget.movieId,
        season: season,
        episode: episode,
      );
  ...
  void _setupInitialSubtitles() {
    ...
    void applyPref(bool isTop) {
      final prefPrefix = isTop ? 'pref_sub_top' : 'pref_sub_bottom';
      final prefLang =
          prefs.getString('${prefPrefix}_lang') ??
          (isTop ? 'english' : 'vietnamese');
      ...
      if (prefLang.startsWith('translate_')) {
        ...
        final tracks = _availableSubtitles
            .where(
              (s) =>
                  _getBaseLanguage(s).toLowerCase() == prefLang.toLowerCase(),
            )
            .toList();
  ...
  Future<void> _fetchSubtitles(SubtitleTrack sub, bool isTop) async {
    try {
      ...
      final tracks = _availableSubtitles
          .where((s) => _getBaseLanguage(s).toLowerCase() == baseLang)
          .toList();
  ...
  Future<void> _translateSubtitles(
    SubtitleTrack targetSub,
    bool isTop, {
    ...
    try {
      ...
      final prefPrefix = isTop ? 'pref_sub_top' : 'pref_sub_bottom';
      final srcLang = getIt<SharedPreferences>().getString(
        '${prefPrefix}_trans_lang',
      );
      final srcTrackIdx =
          getIt<SharedPreferences>().getInt('${prefPrefix}_trans_track') ?? 0;
      ...
      if (srcLang != null) {
        final tracks = _availableSubtitles
            .where(
              (s) => _getBaseLanguage(s).toLowerCase() == srcLang.toLowerCase(),
            )
            .toList();
        if (tracks.isNotEmpty) {
      ...
      final response = await getIt<Dio>().get(sourceSub.src);
      ...
      for (var cue in parsedCues) {
        if (currentCharCount + cue.text.length > 3000) {
          final translatedTexts = await translator.translateBatch(
            currentBatch,
          ...
          for (int i = 0; i < currentCueBatch.length; i++) {
            translatedCues.add(
              SubtitleCue(
                ...
                text: i < translatedTexts.length
                    ? translatedTexts[i]
                    : currentCueBatch[i].text,
              ),
            );
          }
          currentBatch.clear();
  ...
  void _videoListener() {
    ...
    final posMsBottom =
        _controller.value.position.inMilliseconds - _bottomSubDelayMs;
    ...
    if (_primaryCues.isNotEmpty) {
      _lastPrimaryIndex = _findActiveCueIndex(
        _primaryCues,
        posMsTop,
        _lastPrimaryIndex,
      );
    ...
    if (_secondaryCues.isNotEmpty) {
      _lastSecondaryIndex = _findActiveCueIndex(
        _secondaryCues,
        posMsBottom,
        _lastSecondaryIndex,
      );
    ...
    bool positionChangedSignificantly =
        _showControls &&
        (_position.inMilliseconds - _controller.value.position.inMilliseconds)
                .abs() >
            500;
    ...
    if (_primaryCues.isNotEmpty) {
      ...
      final posMsTts =
          _controller.value.position.inMilliseconds - ttsDelayMs.toInt();

      _lastTtsIndex = _findActiveCueIndex(
        _primaryCues,
        posMsTts,
        _lastTtsIndex,
      );
        ...
        if (_isVoiceOverEnabled && _isPlaying) {
          ...
          for (int i = 1; i <= 3; i++) {
            ...
            if (nextIndex < _primaryCues.length) {
              ...
              final nextCleanText = nextCue.text
                ...
                if (currentSubLang != null && currentSubLang != targetLang) {
                  if (!_ttsTranslationCache.containsKey(nextCleanText)) {
                    getIt<TranslationService>()
                        .translate(nextCleanText, targetLang: targetLang)
                        .then((translated) {
      ...
      if (cleanText.isNotEmpty) {
        if (currentSubLang != null && currentSubLang != targetLang) {
          ...
          if (_ttsTranslationCache.containsKey(cleanText)) {
            ...
            getIt<TranslationService>()
                .translate(cleanText, targetLang: targetLang)
                .then((translated) {
                  if (mounted) {
    ...
    if (_duration.inMilliseconds > 0 &&
        _controller.value.position >= _duration &&
        _autoNext &&
        !_isChangingEpisode) {
      _playNextEpisode();
    }
  ...
  void _playNextEpisode() {
    ...
    final sortedEpisodes = widget.allEpisodes.toList()
      ..sort((a, b) {
  ...
  void _changeEpisode(
    ...
    int? customStartPositionMs,
    int nextRetryCount = 0,
  }) async {
    final startPos =
        customStartPositionMs ?? (isAutoRetry ? _position.inMilliseconds : 0);
    setState(() {
    ...
    try {
      ...
      if (streamInfo != null) {
        final hasMultipleSeasons =
            widget.allEpisodes.map((e) => e.season).toSet().length > 1;
        String titleStr = nextEp.title ?? 'Episode ${nextEp.number.toInt()}';
        if (hasMultipleSeasons &&
            !titleStr.toUpperCase().startsWith('S${nextEp.season}')) {
  ...
  Future<void> _clearAllStreamCache() async {
    ...
    await StreamInfoCache.clearCache(
      widget.movieId,
      widget.episodeId,
      widget.streamInfo.currentServerId,
    );
  ...
  void _handlePlayerError(String error) async {
    ...
    if (widget.retryCount < 2) {
      ...
      getIt<LogService>().log(
        '[PlayerScreen] Auto-retrying dead stream (Attempt ${widget.retryCount + 1}/2)...',
      );
      ...
      if (widget.allEpisodes.isEmpty) {
        ...
        ep = widget.allEpisodes.firstWhere(
          (e) => e.id == widget.episodeId,
          orElse: () => widget.allEpisodes.first,
        );
      }
      _changeEpisode(
        ep,
        isAutoRetry: true,
        customStartPositionMs: resumePos,
        nextRetryCount: widget.retryCount + 1,
      );
    } else {
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              _changeEpisode(
                ep,
                customStartPositionMs: _positionBeforeErrorMs,
                nextRetryCount: 0,
              );
            },
                        ...
                        child: GestureDetector(
                          onDoubleTap: () {
                            ...
                            _seekDebounceTimer = Timer(
                              const Duration(milliseconds: 400),
                              () {
                                _controller.seekTo(_targetSeekPosition).then((
                                  _,
                                ) {
                                  if (mounted) {
                      ...
                      Expanded(
                        child: GestureDetector(
                          onDoubleTap: () {
                            ...
                            _targetSeekPosition += const Duration(seconds: 10);
                            _seekAccumulator += 10;
                            if (_targetSeekPosition > _duration)
                              _targetSeekPosition = _duration;
                            setState(() {});
                    ...
                    child: FocusScope(
                      ...
                      child: Builder(
                        builder: (context) {
                          final sortedEpisodes = widget.allEpisodes.toList()
                            ..sort((a, b) {
                              int seasonCompare = (a.season ?? 1).compareTo(
                                b.season ?? 1,
                              );
                              if (seasonCompare != 0) return seasonCompare;
                              return a.number.compareTo(b.number);
  ...
  Widget _buildSubtitleRow({
    ...
    final delayText = delayMs > 0
        ? '+$delayMs ms'
        : (delayMs < 0 ? '$delayMs ms' : '0 ms');
    ...
    if (sub != null) {
      ...
      if (sub.src.startsWith('translate_')) {
        ...
        final prefPrefix = isTop ? 'pref_sub_top' : 'pref_sub_bottom';
        final srcLang =
            getIt<SharedPreferences>().getString('${prefPrefix}_trans_lang') ??
            'English';
        final srcTrackIdx =
            getIt<SharedPreferences>().getInt('${prefPrefix}_trans_track') ?? 0;

        sourceLangStr =
            '${srcLang[0].toUpperCase()}${srcLang.substring(1)} - Track ${srcTrackIdx + 1}';
      } else {
        final tracksForLang = _availableSubtitles
            .where((s) => _getBaseLanguage(s) == currentLanguage)
            .toList();
                  ...
                  return ListTile(
                    ...
                    onTap: () {
                      ...
                      if (lang.contains('(Translate)')) {
                        final target = lang.startsWith('Vietnamese')
                            ? 'vi'
                            : 'en';
                        final tSub = SubtitleTrack(
                          id: -1,
                          label: lang,
                          src: 'translate_$target',
                        );
                        _applySubtitle(tSub, isTop, trackIndex: 0);
                      } else {
                        final tracks = _availableSubtitles
                            .where((s) => _getBaseLanguage(s) == lang)
                            .toList();
                        if (tracks.isNotEmpty) {
  ...
  void _showTrackSelector(BuildContext context, bool isTop, String language) {
    final tracks = _availableSubtitles
        .where((s) => _getBaseLanguage(s) == language)
        .toList();
  ...
  void _showTranslateSourceSelector(
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final peers = _availableSubtitles
                      .where((s) => _getBaseLanguage(s) == baseLang)
                      .toList();
                  final localIdx = peers.indexWhere((s) => s.id == srcSub.id);

                  return ListTile(
                    title: Text(
                      '$baseLang - Track ${localIdx + 1}',
                      style: const TextStyle(color: Colors.white),
                    ),
                    onTap: () {
                      Navigator.pop(context);
                      ...
                      getIt<SharedPreferences>().setString(
                        '${prefPrefix}_trans_lang',
                        baseLang.toLowerCase(),
                      );
                      getIt<SharedPreferences>().setInt(
                        '${prefPrefix}_trans_track',
                        localIdx,
                      );
  ...
  Widget _buildEpisodesDialog(BuildContext context) {
    ...
    final scrollController = ScrollController(
      initialScrollOffset: currentIndex > 0 ? currentIndex * 56.0 : 0,
    );
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final hasMultipleSeasons =
                      widget.allEpisodes.map((e) => e.season).toSet().length >
                      1;
